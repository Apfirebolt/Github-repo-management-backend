...
from .serializers import (
    RepoSerializer,
    RepoStarSerializer,
    RepoUserModelSerializer,
    DeleteRepoSerializer,
)
...
class RepoUserModelCreate(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        serializer = self.get_serializer(data=request.data)
        # raise error if repository already exists for this user
        if RepoUserModel.objects.filter(
            user_name=request.data["user_name"], owner=request.user
        ).exists():
            return Response(
        ...
        headers = self.get_success_headers(serializer.data)
        return Response(
            {"message": "User Profile created successfully", "data": serializer.data},
            status=201,
            headers=headers,
        )


class RepoUserModelDelete(generics.DestroyAPIView):
    serializer_class = DeleteRepoSerializer
    queryset = RepoUserModel.objects.all()
    permission_classes = [IsAuthenticated]
